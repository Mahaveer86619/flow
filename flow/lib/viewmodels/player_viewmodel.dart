...
/// Manages all playback state: current song, play/pause, queue, progress,
/// liked songs, volume, and recently played history.
class PlayerViewModel extends ChangeNotifier {
  ...
  String get currentTimeString {
    ...
    final secs = (_currentSong!.duration.inSeconds * _progress).round();
    return '${secs ~/ 60}:${(secs % 60).toString().padLeft(2, '0')}';
  }

  String get totalTimeString {
    if (_currentSong == null) return '0:00';
    final secs = _currentSong!.duration.inSeconds;
    return '${secs ~/ 60}:${(secs % 60).toString().padLeft(2, '0')}';
  }

  // ── Playback commands ────────────────────────────────────────────────────────

  /// Play a list of songs as a queue, starting at [startIndex].
  ...
  /// Play a single song (creates a one-item queue).
  void play(Song song) {
    _queue = [song];
    _queueIndex = 0;
    _playSong(song);
