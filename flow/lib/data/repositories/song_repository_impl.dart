...
class SongRepositoryImpl implements SongRepository {
  ...
  Future<List<Song>> getPlaylistTracks(String playlistId) async {
    AppLogger.i(_tag, 'getPlaylistTracks($playlistId)');
    try {
      final models = await _source.fetchPlaylistTracks(playlistId);
      final tracks = models.map((m) => m.toEntity()).toList();
      AppLogger.d(_tag, 'getPlaylistTracks($playlistId): ${tracks.length} tracks');
      return tracks;
    } on AppException {
      rethrow;
    } catch (e, st) {
