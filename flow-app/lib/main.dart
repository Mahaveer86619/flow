...
void main() async {
  ...
  // Keep a typed reference to the API source for auth operations.
  final apiSource = ApiSongDataSource(baseUrl: baseUrl);
  final dataSource = useMock ? MockSongDataSource() : apiSource;
  ...
  runApp(
    MultiBlocProvider(
      providers: [
        ...
        BlocProvider(create: (_) => AuthCubit(api: apiSource)),
        BlocProvider(
          create: (_) =>
              PlayerBloc(streamBaseUrl: baseUrl, songRepository: repository),
        ),
        BlocProvider(create: (_) => HomeCubit(getHomeData: getHomeData, dataSource: dataSource)),
        BlocProvider(
